...
class Visualizer:
    ...
    def plot_data(self, X_tf, Y_tf, dates, title, mean, std, filename):
        ...
        plt.plot(dates_formatted, Y_tf, "--", color=tum_colors["line1"], label=title)
        plt.xlabel('Date')
    ...
    def plot_GP(self, X_tf, Y_tf, f_mean, f_cov, title, filename):
        ...
        plt.plot(X_tf, f_mean, color=tum_colors['line1'], label="Predicted Mean")
        plt.plot(X_tf, f_lower, "--", color=tum_colors['line2'], label="95% confidence")
        plt.plot(X_tf, f_upper, "--", color=tum_colors['line3'])
