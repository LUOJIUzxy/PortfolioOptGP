...
class StockPredictor:
    ...
    def process_data(self, ticker, period):
        ...
        X_tf, Y_tf, dates, mean, std = self.normalize_and_reshape(df, column='intraday_return')
    ...
    def plot_pred_data(self, X_daily_tf, Y_daily_tf, X_combined_future, f_mean, f_lower, f_upper, y_mean, y_lower, y_upper, title, mean, std, filename):
        ...
        plt.xlabel('Date')
        plt.ylabel('Normalized Intrady Return')
        plt.title(f'GP Regression on {title} Intrady Return')
        plt.legend()
        plt.grid(True)
        plt.savefig(filename)
        plt.close()
...
_lambda = 0.1
predictor = StockPredictor(tickers, train_start_date, train_end_date, test_start_date, test_end_date, kernel_combinations, inducing_points_svgp, _lambda)
